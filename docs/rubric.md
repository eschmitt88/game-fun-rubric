...
updated: "2026-08-25"
sources_status: "v0.3 — 31 literature notes. Every criterion carries an evidence tier (E1–E5) and citekeys into literature/. Weights remain provisional; see docs/analysis/2026-08-25-evidence-synthesis.md."
lineage: "Structural descendant of GameFlow (sweetser2005gameflow) and Schell's lenses; adds hard gates, behavioural 0–4 anchors, weights, evidence tiers, functional→psychosocial gating, target-profile scoring, expectation calibration."
---

# Game Fun Rubric — v0.3 (single-player digital, genre-agnostic)
...
archetypes (meier2012interesting, E5) are folk colour only.
...
leaderboard rank alone (bowey2015manipulating, E2). Pair every self-report
...
- **G1 — Core loop is fun in isolation.** Strip art, story, meta-progression,
  ...
  directional) — the toggle is a diagnostic, not a target.
- **G2 — Interesting decisions exist.** Meier's criteria: a real trade-off,
  ...
  *between a blind guess and a solved line* (burgun2015why, E5); Costikyan's
  ...
  is not enough (malone1981toward r=.65 is about presence).
...
|---|-----------|------|-------:|-----------------|-----------------|
| 1 | Learning & mastery | functional | 20% | competence = strongest PENS predictor (E2); mastery↔competence r=.88 (E2) | ryan2006motivational, vandenabeele2020development, koster2012theory, cook2007chemistry, deterding2015lens |
| 2 | Agency & meaningful choice | psychosocial | 15% | autonomy predicts enjoyment in 4+ studies (E2); decision-making challenge is a distinct validated factor (E2) | ryan2006motivational, oliver2016video, denisova2020measuring, tyack2020self, chen2007flow |
| 3 | Challenge–skill balance & flow | functional | 15% | challenge construct validated, 4-component (E2); **pooled difficulty main effect null** (E1); balanced vs boring indistinguishable on flow self-report (E1) | vandenabeele2020development, denisova2020measuring, caroux2023player, klarkowski2015operationalising, chen2007flow, juul2013art |
| 4 | Feel & feedback | functional | 15% | music g=.60 is the only significant pooled design effect (E1); juice dose–response is an inverted U (E1); goal-legibility feedback β=.77 (E1) | caroux2023player, kao2020effects, malone1981toward, hicks2018good, jonasson2012juice |
...
| 8 | Clarity, friction & expectation | functional | 5% | intuitive controls fully mediated by competence/autonomy → subtractor only (E2); frustration = expectation–event delta (E3) | ryan2006motivational, ballou2023just, deterding2015joys, sweetser2005gameflow |
...
| 1.2 **Skill atoms chain** — each Action→Simulation→Feedback→Model loop unlocks a next one; three independent formalizations converge (Cook 2007, Koster/Humble 2012, Deterding 2015) | skills isolated | linear chain | branching skill graph; mastery visible in play | E4 (convergent) |
| 1.3 **Feedback lets the model update** — player can tell *why* and *how much* they succeeded/failed; failure feels deserved | outcomes feel random | usually clear | every failure teaches a specific lesson; players blame themselves and are right to | **E1** (juul2013art p<.016; PXI Progress Feedback CR=.92; hicks2018good magnitude) |
| 1.4 **Expression of mastery** — a skilled player *visibly* plays differently (behavioural, not self-report) | no | somewhat | strongly; spectators can tell | E2 (PXI Mastery↔PENS Competence r=.88; bowey2015manipulating caution) |
...
of options triggers disengagement (kumari2019role, E3).
...
| 2.5 **Self-directed play** — room for the *target* motivation profile to set its own goals; solitary play's freedom from emotion-display labour is part of this autonomy (deterding2015joys) | strictly on rails | optional side goals | sandbox-level self-direction inside structure, aimed at the S1 profile | E2 (PXI Autonomy; yee2015handy Discovery) / E3 (deterding2015joys) |
...
affect check. "Challenge" itself is four largely independent things
(denisova2020measuring, E2): score calibration **per type** when the game
mixes them.

| Criterion | 0 | 2 | 4 | Tier |
|---|---|---|---|---|
| 3.1 **Difficulty tracks skill, as an irregular wave, per challenge type** — guaranteed exposure to both failure and success; adaptation embedded in the mechanic (which depth/wave to engage) ranks above a settings menu | flat or spiky | reasonable curve; menu difficulty select | wave matched to observed skill for each challenge type present; self-adjustment inside play | E2 (PXI Challenge; CORGIS) / E4 (Chen, Falstein) / E1-null caveats |
| 3.2 **Failure cost is calibrated** — score *which* punishment (energy, life, termination, setback) and how much; tolerance tracks the player's *confidence* in closing the gap, not gap size (to2016integrating); permanent *narrative* loss is scored under 7, not here | players report wanting to quit (ballou2023just ladder: rush → adapt → disengage → quit) | okay | retry instant, setback punishment minimal, loss felt, player wants another go | E3 (juul2013art; ballou2023just; to2016integrating) |
| 3.3 **Sense of control** — inputs map reliably to outcomes; no unfair randomness; no interruptive choice friction; no "unfair situations" (illegitimately advantaged AI) that push blame outward | random death, input lag, forced menus, cheating AI | mostly fair | player always blames themselves, and is right to | **E1** (juul2013art) / E3 (ballou2023just); caution: control self-report tracks *ease* (klarkowski2015operationalising) |
...
| 3.5 **Session shape** — a "one more" loop hook + natural arc stopping points; curiosity beyond the obstacle, identity, and rewards carry players through frustrating segments | neither | one | both | E4 (loops-and-arcs) / E3 (ballou2023just) |
...
enjoyment (g=.60); SFX alone are not (caroux2023player, E1).
...
| 4.2 **Acknowledged, legible, then juicy** — every input is acknowledged ("I heard you" — meier2012interesting); the state change is readable *before* the screenshake; then layered hit-stop, particles, squash-stretch, sound, numbers, proportional — with a ceiling: extreme juice hurts | silent, dry — or overwhelming | some effects | acknowledgment + goal legibility unmistakable + layered juice at medium/high, toggleable for testing | E1 (malone1981toward; kao2020effects) / E3 (hicks2018good) / E4 (jonasson2012juice) |
...
| 4.4 **State legibility** — HP, resources, threats readable at a glance; juice directs attention rather than dividing it ("Glanceable", "Focus of Attention") | must read menus, or juice obscures state | mostly | diegetic or instant | E3 (hicks2018good) / E4 (deterding2015lens lenses); quantitative trade-off still unresolved |
| 4.5 **Audio and aesthetic coherence** — *has music*, not only SFX; thematic and gameplay coherence: audio/visual/tone reinforce the fantasy | no music, clashing | coherent, music present | distinctive style; music carries mood | **E1** (music g=.60) / E2 (PXI Audiovisual Appeal) / E3 (hicks2018good coherence) |
...
| 5.2 **Uncertain outcome** — uses ≥2 of Malone's four mechanisms; never sure of success or failure | outcomes obvious | some tension | tension sustained to the end of each arc | E1 (malone1981toward) / E3 (kumari2019role) |
...
| 6.3 **Information gaps** — the game plants incompleteness the player wants resolved, both *content* gaps (entirely new things) and *configuration* gaps (novel arrangements of known things) | no mystery | some | constant, well-paced reveals of both kinds | E1 (malone) / E2 (PXI Curiosity) / E3 (to2016integrating; kumari2019role) |
...
led the player to expect (ballou2023just, E3).

| Criterion | 0 | 2 | 4 | Tier |
|---|---|---|---|---|
| 8.1 **Onboarding targets the real skill floor** — teaches by doing in the first minute ("Varied Onboarding"); neither re-teaches known skills (boredom burnout) nor assumes absent ones (frustration burnout) | wall of text, or mistargeted | tutorial level | teaching invisible inside play, calibrated to the audience | E4 (cook2007chemistry; deterding2015lens) |
...
| 8.5 **Accessibility of difficulty/controls** — options broaden who reaches flow; redundant multimodal feedback doubles as accessibility (DDA has no pooled enjoyment effect, g=.19 ns) | none | basic | options broaden reach without menu friction | E1-null caution / E3 (hicks2018good) |
| 8.6 **Expectation calibration** *(new in v0.3)* — the game signals upcoming difficulty, constraint and randomness so expectations track what the mechanic will do (Candy Crush's up-front "hard level" labels); surprises are designed, not accidental | expectations routinely violated; players report "unfair" | mostly signposted | players are never blindsided by the *kind* of challenge, only by its content | E3 (ballou2023just) |
...
4. **Per-playtest**: 2–3 raters score independently, discuss deltas ≥ 2.
   ...
   responses (juul2013art). If challenge types are mixed, add CORGIS.
...
## Known gaps (updated v0.3)

- **Weights**: still unconfirmed. Only music (g=.60), juice dose (inverted U)
  and control responsiveness (trend) survive controlled/pooled testing; the
  challenge→enjoyment link has never been tested with a multi-component
  challenge measure (CORGIS deliberately excluded enjoyment). Needs: a
  factorial study varying ≥3 design factors with PXI + behavioural retention.
- **Juice vs legibility** (4.2 ↔ 4.4): now sourced qualitatively
  (hicks2018good; deterding2015lens "Juicy"/"Glanceable") and directionally
  (kao2020effects: extreme juice hurts) but *why* extreme juice hurts —
  legibility, distraction, overload — is not distinguishable from the
  abstract. Re-fetch Kao 2020 full text via library access.
- **Rater reliability**: unknown; no inter-rater data for GameFlow either.
...
- **Social / People Factor**: deliberately unweighted; relatedness is an
  independent predictor (β=.12–.18, ryan2006motivational) for multiplayer,
  and *may* apply to single-player worlds/companions (hypothesis).
- **Fun vs compulsion**: retention mechanics that are not fun (variable
  reward, loss aversion, loot boxes — zendle2018 boundary) stay outside.
- **Abstract-only sources**: kao2020effects, bowey2015manipulating and
  deterding2015lens main text were not retrievable; their tiers are
  provisional.
